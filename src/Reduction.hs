...
import Text.Parsec (Parsec, digit, letter, many1, parse, string)
...
solve4 n0 m (name, oneRate, halfRate) =
  ...
  where
    go !acc !n =
      let half_quantity = n `div` 2 :: Int
          oneByOne = (n - half_quantity) * oneRate
       in -- trace (show (name, acc, n)) $
...
formatInput (row : rows) =
  let n : m : l : _ = words row <&> read @Int
